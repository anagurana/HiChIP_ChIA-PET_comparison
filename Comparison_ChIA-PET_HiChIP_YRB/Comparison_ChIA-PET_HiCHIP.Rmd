...
# Compare if subsets of peaks between technologies are the same
```{r}
...
# Calculate the mean max CTCF motif strength
tmp=vector(mode="list", length=NROW(ctcf_strength_ChIAPET))
names(tmp)=names(ctcf_strength_ChIAPET)
avergae_max_ctcf=list(tmp,tmp,tmp)
names(avergae_max_ctcf)=experiments
rm(tmp)

for (experiment in experiments){
  for (overlap in names(ctcf_strength_ChIAPET)){
    avergae_max_ctcf[[experiment]][[overlap]]=mean(get(paste0("ctcf_strength_", experiment))[[overlap]]$max_motif_CTCF)
  }
}
