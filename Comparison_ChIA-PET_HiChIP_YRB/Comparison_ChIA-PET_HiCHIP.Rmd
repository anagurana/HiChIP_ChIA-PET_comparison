...
for (cell_line in trios){
  png(paste0('plots/venn_diagrams/Common_raw_peaks_ChIA-PET_', cell_line, '_different_replicates.png'), res = 300, width = 2400, height = 2400)
  plot.new()
    
  venndiag=VennDiagram::draw.pairwise.venn(NROW(IRanges::reduce(c(rep2_peaks_with_motif_ChIAPET[[paste0(cell_line,"_rep1")]], 
                                                                   rep2_peaks_with_motif_ChIAPET[[paste0(cell_line,"_rep2")]]))),
                                            NROW(peaks_with_motif_ChIAPET[[cell_line]]),
                                            NROW(subsetByOverlaps(peaks_with_motif_ChIAPET[[cell_line]],
                                                                  IRanges::reduce(c(rep2_peaks_with_motif_ChIAPET[[paste0(cell_line,"_rep1")]], 
                                                                                    rep2_peaks_with_motif_ChIAPET[[paste0(cell_line,"_rep2")]])))
                                                 ),
                                                scaled = T,
                                                category=c("Single replicates", "Merged replicates"),
                                                print.mode=c('raw','percent'),
                                                fill=c("lightskyblue", color("chiapet")),
                                                rotation.degree=0,
                                                lwd = 3,
                                                cat.cex = 2,
                                                cat.pos = c(240,120),
...
# HiChIP
for (cell_line in trios){
  png(paste0('plots/venn_diagrams/Common_raw_peaks_HiChIP_', cell_line, '_different_replicates.png'), res = 300, width = 2400, height = 2400)
  plot.new()
  
  venndiag=VennDiagram::draw.pairwise.venn( NROW(IRanges::reduce(c(rep2_peaks_with_motif_HiChIP[[paste0(cell_line,"_rep1")]], 
                                                                   rep2_peaks_with_motif_HiChIP[[paste0(cell_line,"_rep2")]]))), 
                                            NROW(peaks_with_motif_HiChIP[[cell_line]]), 
                                            NROW(subsetByOverlaps(peaks_with_motif_HiChIP[[cell_line]],
                                                                  IRanges::reduce(c(rep2_peaks_with_motif_HiChIP[[paste0(cell_line,"_rep1")]], 
                                                                                    rep2_peaks_with_motif_HiChIP[[paste0(cell_line,"_rep2")]])))),
                                                scaled = T,
                                                category=c("Single replicates", "Merged replicates"),
                                                print.mode=c('raw','percent'),
                                                fill=c("darkolivegreen1", color("hichip")),
                                                rotation.degree=0,
                                                lwd = 3,
                                                cat.cex = 2,
                                                cat.pos = c(240,120),
  ...
  for (experiment in experiments){
    ...
    for (overlap in names(GM19238_common_peaks_3_tech_red)){
      TPM_and_FPKM_values[[cell_line]][[experiment]][[overlap]]=
        calculate_FPKM_TPM(subsetByOverlaps(get(paste0("peaks_reduced_with_motif_coverage_", experiment))[[cell_line]], 
...
for (cell_line in trios){
  # png(paste0("plots/peaks/TPM_value_", cell_line,"_violin.png"), res = 300, width = 5000, height = 3000)
  # plot.new()
  # plots=list()
  for (experiment in experiments){
    png(paste0("plots/peaks/TPM_value_", cell_line,"_",experiment,"_violin.png"), res = 300, width = 5000, height = 3000)
    plot.new()
    ...
    # plots[[cell_line]][[experiment]][[overlap]]=p
    # assign(paste0("p_", experiment), p)
  }
  # (p_ChIAPET)/(p_HiChIP)/(p_ChIPSeq)

  
  # print(plots[[cell_line]][[experiment]])
  # p1/p2/p3
  # 
  # (p6+(p1 | p2) / (p3 | p4 | p5) + plot_layout(widths=c(1, 2)))
...
# Make violin plots only nessesary
```{r}
trios<<-c("GM19238")
experiments=c("ChIAPET", "HiChIP", "ChIPSeq")
# Orders of overlaps from from names(ctcf_strength_GM19238)
