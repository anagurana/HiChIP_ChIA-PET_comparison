...
CTCF.hg38 = read.table(file.path(abs_path, "CTCF_hg38.txt"))
...
for (cell_line in cell_lines){
  ...
  # Draw the plots
  png(paste0('Common_peaks_ChIA-PET_', cell_line, '_replicates.png'), res = 300, width = 2400, height = 2400)
  plot.new()
...
for (cell_line in cell_lines){
  write.table(unname(data.frame(seqnames(peaks_with_motif_HiChIP[[cell_line]]), 
              ...
              paste0(abs_path, "/hichip/", "peaks_HiChIP_", cell_line, ".bed"),
              sep="\t",row.names=FALSE, quote = FALSE)
}
...
write.table(unname(data.frame(seqnames(hichip_bins),
            ...
            paste0(abs_path, "/hichip/bins_HiChIP.bed"),
            sep="\t",row.names=FALSE, quote = FALSE)
...
for (cell_line in cell_lines){
  write.table(unname(data.frame(seqnames(rep2_peaks_all_with_motif_HiChIP[[cell_line]]), 
              ...
              paste0(abs_path, "/hichip/", cell_line, "/peaks_HiChIP_", cell_line, ".bed"),
              sep="\t",row.names=FALSE, quote = FALSE)
}
...
```{r}
experiment="hichip"
norm_inp_hichip_2rep=data.frame(read.table(paste0(abs_path, "/", experiment, "/hichip_2rep_mrg_bins_profile_bins.xls"), header = T))
cell_lines=c("GM19238_rep1", "GM19238_rep2", "GM19239_rep1", "GM19239_rep2","GM19240_rep1","GM19240_rep2")
...
pos_false_tables_hichip=list(
  create_fp_tables_and_save_graphs(common_hichip_manorm[[1]],trios[1:2], experiment, "all_mrgBins", par),
  create_fp_tables_and_save_graphs(common_hichip_manorm[[4]],trios[1:2], experiment, "mean>=10_mrgBins", par),      
  create_fp_tables_and_save_graphs(common_hichip_manorm[[5]],trios[1:2], experiment, "mean<10_mrgBins", par),
  create_fp_tables_and_save_graphs(common_hichip_manorm[[2]],trios[c(1,3)], experiment, "all_mrgBins", par),
  create_fp_tables_and_save_graphs(common_hichip_manorm[[6]],trios[c(1,3)], experiment, "mean>=10_mrgBins", par),
  create_fp_tables_and_save_graphs(common_hichip_manorm[[7]],trios[c(1,3)], experiment, "mean<10_mrgBins", par),
  create_fp_tables_and_save_graphs(common_hichip_manorm[[3]],trios[2:3], experiment, "all_mrgBins", par),
  create_fp_tables_and_save_graphs(common_hichip_manorm[[8]],trios[2:3], experiment, "mean>=10_mrgBins", par),
  create_fp_tables_and_save_graphs(common_hichip_manorm[[9]],trios[2:3], experiment, "mean<10_mrgBins", par))
...
for (i in 1:NROW(confusion_tables_hichip)){
  confusion_tables_hichip[i]=list(caret::confusionMatrix(factor(common_hichip_manorm[[names(confusion_tables_hichip)[i]]]$common_padj, levels=c(TRUE, FALSE)), 
...
Select possible constant peaks
```{r}
manorm_find_const_peaks=function(pos_faalse_table, norm){
  
}
# Between 38 and other cell_lines
# 7830 vs 5220 --> 4394 common
f1=intersect(row.names(pos_false_tables_hichip$GM38_GM39$false_peaks[pos_false_tables_hichip$GM38_GM39$false_peaks$common_padj==TRUE,]),
          row.names(pos_false_tables_hichip$GM38_GM40$false_peaks[pos_false_tables_hichip$GM38_GM40$false_peaks$common_padj==TRUE,]))
#1341
t1=intersect(row.names(pos_false_tables_hichip$GM38_GM39$GM38T_GM39T[pos_false_tables_hichip$GM38_GM39$GM38T_GM39T$common_padj==FALSE,]),
          row.names(pos_false_tables_hichip$GM38_GM40$GM38T_GM40T[pos_false_tables_hichip$GM38_GM40$GM38T_GM40T$common_padj==FALSE,]))


# Between 39 and other cell_lines
# 7830 vs 7569 --> 6474 common
f2=intersect(row.names(pos_false_tables_hichip$GM38_GM39$false_peaks[pos_false_tables_hichip$GM38_GM39$false_peaks$common_padj==TRUE,]),
          row.names(pos_false_tables_hichip$GM39_GM40$false_peaks[pos_false_tables_hichip$GM39_GM40$false_peaks$common_padj==TRUE,]))
#1027
t2=intersect(row.names(pos_false_tables_hichip$GM38_GM39$GM38T_GM39T[pos_false_tables_hichip$GM38_GM39$GM38T_GM39T$common_padj==FALSE,]),
          row.names(pos_false_tables_hichip$GM39_GM40$GM39T_GM40T[pos_false_tables_hichip$GM39_GM40$GM39T_GM40T$common_padj==FALSE,]))

# Between 40 and other cell_lines
# 5220 vs 7569 --> 4412 common
f3=intersect(row.names(pos_false_tables_hichip$GM38_GM40$false_peaks[pos_false_tables_hichip$GM38_GM40$false_peaks$common_padj==TRUE,]),
          row.names(pos_false_tables_hichip$GM39_GM40$false_peaks[pos_false_tables_hichip$GM39_GM40$false_peaks$common_padj==TRUE,]))
#2568
t3=intersect(row.names(pos_false_tables_hichip$GM38_GM40$GM38T_GM40T[pos_false_tables_hichip$GM38_GM40$GM38T_GM40T$common_padj==FALSE,]),
          row.names(pos_false_tables_hichip$GM39_GM40$GM39T_GM40T[pos_false_tables_hichip$GM39_GM40$GM39T_GM40T$common_padj==FALSE,]))

# Dedublicate the peaks
# 7472 peaks are constant
id_constant_hichip_by_manorm_peaks=data.frame(peak=unique(c(f1, f2, f3)), isMAnorm=TRUE, isSPP=FALSE)


constant_hichip_by_manorm_peaks=norm_hichip_2rep[id_constant_hichip_by_manorm_peaks,]
constant_hichip_by_manorm_peaks$by_spp=FALSE
constant_hichip_by_manorm_peaks$by_manorm=TRUE

id_constant_hichip_by_spp_peaks=data.frame(peak=row.names(pos_false_tables_hichip$GM38_GM39$GM38T_GM39T), isMAnorm=TRUE,isSPP=TRUE)
cos=rbind(id_constant_hichip_by_manorm_peaks,id_constant_hichip_by_spp_peaks )

unique(cos$peak)
ifelse(duplicated(cos), cos$isMAnorm=TRUE, cos$isSPP=TRUE)

unique(id_constant_hichip_by_manorm_peaks$peak, id_constant_hichip_by_spp_peaks$peak)

constant_hichip_by_spp_peaks=norm_hichip_2rep[id_constant_hichip_by_spp_peaks,]
constant_hichip_by_spp_peaks$by_spp=TRUE
constant_hichip_by_spp_peaks$by_manorm=FALSE

rbind(constant_hichip_by_manorm_peaks, constant_hichip_by_spp_peaks) 

rbind(constant_hichip_by_manorm_peaks, constant_hichip_by_spp_peaks)

constant_hichip_with_manorm=(constant_hichip_by_manorm_peaks, constant_hichip_by_spp_peaks))
  
unlist(true_common_peaks_ChIAPET, recursive = TRUE, use.names = TRUE), unlist(true_common_peaks_ChIPSeq, recursive = TRUE, use.names = TRUE))
  


all_id_constant_hichip_whith_manorm_peaks=unique(c(id_constant_hichip_by_manorm_peaks, id_constant_hichip_by_spp_peaks))
constant_hichip_with_manorm=norm_hichip_2rep[all_id_constant_hichip_whith_manorm_peaks,]

cos.df=data.frame(constant_hichip_with_manorm %>% select(chrom, start, end),
                  name=row.names(constant_hichip_with_manorm),
                  padj_38_39=res_hichip_rep2$GM19238_GM19239[all_id_constant_hichip_whith_manorm_peaks,]$padj,
                  padj_38_40=res_hichip_rep2$GM19238_GM19240[all_id_constant_hichip_whith_manorm_peaks,]$padj,
                  padj_39_40=res_hichip_rep2$GM19239_GM19240[all_id_constant_hichip_whith_manorm_peaks,]$padj)
constant_hichip_with_manorm.gr=GRanges(cos.df)

subsetByOverlaps(constant_hichip_with_manorm.gr, true_common_peaks_HiChIP)

cos.gr=GRanges(norm_hichip_2rep[unique(cos$peak),])
subsetByOverlaps(IRanges::reduce(cos.gr+1000), IRanges::reduce(true_common_peaks_reduced_ChIPSeq$constant_peaks+1000))
...
# 4054
id_unconstant_hichip_by_manorm_peaks=unique(c(t1, t2, t3))
norm_hichip_2rep[id_unconstant_hichip_by_manorm_peaks,]



# How many and which peaks from them got the padj<0.001 whith aovBioCond
# 1036 from 7472
norm_hichip_2rep[row.names(res_hichip_rep2$all_trios[id_constant_hichip_by_manorm_peaks,][res_hichip_rep2$all_trios[id_constant_hichip_by_manorm_peaks,]$padj<0.001,]),][1:10,]

# Add others true peaks


```
Select only peaks that are less than 5000 kb
```{r}
...
experiment="hichip"
trios=c("GM19238", "GM19239", "GM19240")
norm_inp_hichip=data.frame(read.table(paste0(abs_path, "/", experiment, "/hichip_merged_profile_bins.xls"), header = T))
...
png(paste0('plots/',experiment,'_MAnorm_merRep_trios_diff_test.png'), res = 300, width = 2200, height = 2200)
par(mar=c(2.3,2.3,2.3,2.3))
layout(matrix(c(1,2,3,4,1,5,3,4,1,6,3,4),ncol=3),heights=c(1,3,1,3,1,3))
...
# Create tables that will include all important information for the analysis
common_hichip_manorm_merged=list(
...
common_hichip_manorm_merged=add_greater_and_lower_mean_ranges_tables(common_hichip_manorm_merged, mean_range, trios[1:2])
common_hichip_manorm_merged=add_greater_and_lower_mean_ranges_tables(common_hichip_manorm_merged, mean_range, trios[c(1,3)])
common_hichip_manorm_merged=add_greater_and_lower_mean_ranges_tables(common_hichip_manorm_merged, mean_range, trios[2:3])

# Create false pos tables
par="isPval"

pos_false_tables_hichip_merged=list(
  create_fp_tables_and_save_graphs(common_hichip_manorm_merged[[1]],trios[1:2], experiment, "all_mrgRep", par),
  create_fp_tables_and_save_graphs(common_hichip_manorm_merged[[4]],trios[1:2], experiment, "mean>=10_mrgRep", par),      
  create_fp_tables_and_save_graphs(common_hichip_manorm_merged[[5]],trios[1:2], experiment, "mean<10_mrgRep", par),
  create_fp_tables_and_save_graphs(common_hichip_manorm_merged[[2]],trios[c(1,3)], experiment, "all_mrgBins", par),
  create_fp_tables_and_save_graphs(common_hichip_manorm_merged[[6]],trios[c(1,3)], experiment, "mean>=10_mrgRep", par),
  create_fp_tables_and_save_graphs(common_hichip_manorm_merged[[7]],trios[c(1,3)], experiment, "mean<10_mrgRep", par),
  create_fp_tables_and_save_graphs(common_hichip_manorm_merged[[3]],trios[2:3], experiment, "all_mrgRep", par),
  create_fp_tables_and_save_graphs(common_hichip_manorm_merged[[8]],trios[2:3], experiment, "mean>=10_mrgRep", par),
  create_fp_tables_and_save_graphs(common_hichip_manorm_merged[[9]],trios[2:3], experiment, "mean<10_mrgRep", par))

names(pos_false_tables_hichip_merged)=names(common_hichip_manorm_merged)[c(1,4,5,2,6,7,3,8,9)]

# Create confusion tables for every pairs of replicates and freater and lower ranges              
confusion_tables_hichip_merged=vector(mode="list", length=NROW(pos_false_tables_hichip_merged))
names(confusion_tables_hichip_merged)=names(pos_false_tables_hichip_merged)
for (i in 1:NROW(confusion_tables_hichip_merged)){
  confusion_tables_hichip_merged[i]=list(caret::confusionMatrix(factor(common_hichip_manorm_merged[[names(confusion_tables_hichip_merged)[i]]]$common_padj, levels=c(TRUE, FALSE)), 
                       factor(common_hichip_manorm_merged[[names(confusion_tables_hichip_merged)[i]]]$predicted_rev, levels=c(TRUE, FALSE)),
                       dnn=c("pval>0.05", "SPP_detected")))
}

# 4 CASES

confusion_data_hichip_merged=list(
       # Both show common (TP)
  list(TP=pos_false_tables_hichip_merged[[1]][[1]][pos_false_tables_hichip_merged[[1]][[1]]$common_pval==TRUE,],
       # Spp pokazuje tak, a padj pokazuje róźną siłę (FP)
       FP=pos_false_tables_hichip_merged[[1]][[1]][pos_false_tables_hichip_merged[[1]][[1]]$common_pval==FALSE,], 
       # SPP pokazuje nie, a padj pokazuje, że siła jest taka sama (FN)
       FN=pos_false_tables_hichip_merged[[1]][["false_peaks"]][pos_false_tables_hichip_merged[[1]][["false_peaks"]]$common_pval==TRUE,],
       # SPP pokazuje nie i padj się zgadza  (TN)
       TN=pos_false_tables_hichip_merged[[1]][["false_peaks"]][pos_false_tables_hichip_merged[[1]][["false_peaks"]]$common_pval==FALSE,]),
  list(TP=pos_false_tables_hichip_merged[[4]][[1]][pos_false_tables_hichip_merged[[4]][[1]]$common_pval==TRUE,],
       
       # Spp pokazuje tak, a padj pokazuje róźną siłę (FP)
       FP=pos_false_tables_hichip_merged[[4]][[1]][pos_false_tables_hichip_merged[[4]][[1]]$common_pval==FALSE,],
       # SPP pokazuje nie, a padj pokazuje, że siła jest taka sama (FN)
       FN=pos_false_tables_hichip_merged[[4]][["false_peaks"]][pos_false_tables_hichip_merged[[4]][["false_peaks"]]$common_pval==TRUE,],
       # SPP pokazuje nie i padj się zgadza  (TN)
       TN=pos_false_tables_hichip_merged[[4]][["false_peaks"]][pos_false_tables_hichip_merged[[4]][["false_peaks"]]$common_pval==FALSE,]),
  
  list(TP=pos_false_tables_hichip_merged[[7]][[1]][pos_false_tables_hichip_merged[[7]][[1]]$common_pval==TRUE,],
       # Spp pokazuje tak, a padj pokazuje róźną siłę (FP)
       FP=pos_false_tables_hichip_merged[[7]][[1]][pos_false_tables_hichip_merged[[7]][[1]]$common_pval==FALSE,],
       # SPP pokazuje nie, a padj pokazuje, że siła jest taka sama (FN)
       FN=pos_false_tables_hichip_merged[[7]][["false_peaks"]][pos_false_tables_hichip_merged[[7]][["false_peaks"]]$common_pval==TRUE,],
       # SPP pokazuje nie i padj się zgadza  (TN)
       TN=pos_false_tables_hichip_merged[[7]][["false_peaks"]][pos_false_tables_hichip_merged[[7]][["false_peaks"]]$common_pval==FALSE,]))

names(confusion_data_hichip_merged)=names(common_hichip_manorm_merged)[1:3]
...
norm_inp_chiapet_2rep=data.frame(read.table(paste0(abs_path, "/", experiment, "/chiapet_2rep_mrg_bins_profile_bins.xls"), header = T))
...
common_chiapet_manorm=add_greater_and_lower_mean_ranges_tables(common_chiapet_manorm, mean_range, trios[1:2])
common_chiapet_manorm=add_greater_and_lower_mean_ranges_tables(common_chiapet_manorm, mean_range, trios[c(1,3)])
common_chiapet_manorm=add_greater_and_lower_mean_ranges_tables(common_chiapet_manorm, mean_range, trios[2:3])

# Create false pos tables                                    
par="isPval"
prefix="_mrgBins_"
pos_false_tables_chiapet=list(
  create_fp_tables_and_save_graphs(common_chiapet_manorm[[1]],trios[1:2], experiment, paste0("all",prefix),par),
  create_fp_tables_and_save_graphs(common_chiapet_manorm[[4]],trios[1:2], experiment, paste0("mean>=10",prefix), par),      
  create_fp_tables_and_save_graphs(common_chiapet_manorm[[5]],trios[1:2], experiment, paste0("mean<10",prefix), par),
  create_fp_tables_and_save_graphs(common_chiapet_manorm[[2]],trios[c(1,3)], experiment, paste0("all",prefix), par),
  create_fp_tables_and_save_graphs(common_chiapet_manorm[[6]],trios[c(1,3)], experiment, paste0("mean>=10",prefix), par),
  create_fp_tables_and_save_graphs(common_chiapet_manorm[[7]],trios[c(1,3)], experiment, paste0("mean<10",prefix), par),
  create_fp_tables_and_save_graphs(common_chiapet_manorm[[3]],trios[2:3], experiment, paste0("all",prefix), par),
  create_fp_tables_and_save_graphs(common_chiapet_manorm[[8]],trios[2:3], experiment, paste0("mean>=10",prefix), par),
  create_fp_tables_and_save_graphs(common_chiapet_manorm[[9]],trios[2:3], experiment, paste0("mean<10",prefix), par))
...
# [ChIA-PET merged] MANorm data processing
```{r}
experiment="chiapet"
trios=c("GM19238", "GM19239", "GM19240")
...
plot.new()
text(0.5,0.5,"Within paris of trios ",cex=3,font=2)
MAplot(res_chiapet[[2]], pval = 0.05, ylim = c(-2, 2), main=paste0(trios[1]," vs. ",trios[2]), line=0.2)
abline(h = 0, lwd = 2, lty = 5, col = "green3")
plot.new()
text(0.5,0.5,"Within all trios ",cex=3,font=2)
plot(res_chiapet[[1]], pval = 0.05,line=0.2)
MAplot(res_chiapet[[3]], pval = 0.05,ylim = c(-2, 2), main=paste0(trios[1]," vs. ",cell_lines[3]), line=0.2)
abline(h = 0, lwd = 2, lty = 5, col = "green3")
MAplot(res_chiapet[[4]], pval = 0.05,ylim = c(-2, 2), main=paste0(cell_lines[2]," vs. ",trios[3]),line=0.2)
abline(h = 0, lwd = 2, lty = 5, col = "green3")
dev.off()
...
# [ChIA-PET merged] Analysis of the results 
```{r}
experiment="ChIA-PET"

# Create tables that will include all important information for the analysis
cell_lines=list(trios[1:2],
                trios[c(1,3)],
                trios[2:3])
common_chiapet_manorm_merged=list(
  create_table_common_peaks_with_info(norm_chiapet, res_chiapet, conds_chiapet, cell_lines[[1]]),
  create_table_common_peaks_with_info(norm_chiapet, res_chiapet, conds_chiapet, cell_lines[[2]]),
  create_table_common_peaks_with_info(norm_chiapet, res_chiapet, conds_chiapet, cell_lines[[3]]))
names(common_chiapet_manorm_merged)=c(
  paste0(paste0(str_sub(trios[1],1,2),str_sub(trios[1],start=6)),"_",paste0(str_sub(trios[2],1,2),str_sub(trios[2],start=6))),
  paste0(paste0(str_sub(trios[1],1,2),str_sub(trios[1],start=6)),"_",paste0(str_sub(trios[3],1,2),str_sub(trios[3],start=6))),
  paste0(paste0(str_sub(trios[2],1,2),str_sub(trios[2],start=6)),"_",paste0(str_sub(trios[3],1,2),str_sub(trios[3],start=6))))
...
# Create additional tables with lower and higher values of mean values (select the squares)
mean_range=c(10,10)
common_chiapet_manorm_merged$GM38_GM39_mean_gr10=common_chiapet_manorm_merged$GM38_GM39[common_chiapet_manorm_merged$GM38_GM39$GM19238.mean>=mean_range[1]&                                                        common_chiapet_manorm_merged$GM38_GM39$GM19239.mean>=mean_range[2],]
common_chiapet_manorm_merged$GM38_GM39_mean_lw10=common_chiapet_manorm_merged$GM38_GM39[common_chiapet_manorm_merged$GM38_GM39$GM19238.mean<mean_range[1]&                                                common_chiapet_manorm_merged$GM38_GM39$GM19239.mean<mean_range[2],]

common_chiapet_manorm_merged$GM38_GM40_mean_gr10=common_chiapet_manorm_merged$GM38_GM40[common_chiapet_manorm_merged$GM38_GM40$GM19238.mean>=mean_range[1]&                                                                        common_chiapet_manorm_merged$GM38_GM40$GM19240.mean>=mean_range[2],]
common_chiapet_manorm_merged$GM38_GM40_mean_lw10=common_chiapet_manorm_merged$GM38_GM40[common_chiapet_manorm_merged$GM38_GM40$GM19238.mean<mean_range[1]&                                                 common_chiapet_manorm_merged$GM38_GM40$GM19240.mean<mean_range[2],]

common_chiapet_manorm_merged$GM39_GM40_mean_gr10=common_chiapet_manorm_merged$GM39_GM40[common_chiapet_manorm_merged$GM39_GM40$GM19239.mean>=mean_range[1]&                                                                        common_chiapet_manorm_merged$GM39_GM40$GM19240.mean>=mean_range[2],]
common_chiapet_manorm_merged$GM39_GM40_mean_lw10=common_chiapet_manorm_merged$GM39_GM40[common_chiapet_manorm_merged$GM39_GM40$GM19239.mean<mean_range[1]&                                                                        common_chiapet_manorm_merged$GM39_GM40$GM19240.mean<mean_range[2],]

pos_false_tables_chiapet_merged=list(
  GM38_GM39=create_fp_tables_and_save_graphs(common_chiapet_manorm_merged$GM38_GM39,trios[1:2], experiment, "all_merRep", "isPval"),
  GM38_GM39_mean_gr10=create_fp_tables_and_save_graphs(common_chiapet_manorm_merged$GM38_GM39_mean_gr10,trios[1:2], experiment, prefix="mean>=10_merRep", padj_or_pval = "isPval"),
  GM38_GM39_mean_lw10=create_fp_tables_and_save_graphs(common_chiapet_manorm_merged$GM38_GM39_mean_lw10,trios[1:2], experiment, "mean<10_merRep", "isPval"),
  GM38_GM40=create_fp_tables_and_save_graphs(common_chiapet_manorm_merged$GM38_GM40,trios[1:3], experiment, "all_merRep", "isPval"),
  GM38_GM40_mean_gr10=create_fp_tables_and_save_graphs(common_chiapet_manorm_merged$GM38_GM40_mean_gr10,trios[1:3], experiment, "mean>=10_merRep", "isPval"),
  GM38_GM40_mean_lw10=create_fp_tables_and_save_graphs(common_chiapet_manorm_merged$GM38_GM40_mean_lw10,trios[1:3], experiment, "mean<10_merRep", "isPval"),
  GM39_GM40=create_fp_tables_and_save_graphs(common_chiapet_manorm_merged$GM39_GM40,trios[2:3], experiment, "all_merRep", "isPval"),
  GM39_GM40_mean_gr10=create_fp_tables_and_save_graphs(common_chiapet_manorm_merged$GM39_GM40_mean_gr10,trios[2:3], experiment, "mean>=10_merRep", "isPval"),
  GM39_GM40_mean_lw10=create_fp_tables_and_save_graphs(common_chiapet_manorm_merged$GM39_GM40_mean_lw10,trios[2:3], experiment, "mean<10_merRep", "isPval"))
               
               
confusion_tables_chiapet_merged=vector(mode="list", length=NROW(pos_false_tables_chiapet_merged))
names(confusion_tables_chiapet_merged)=names(pos_false_tables_chiapet_merged)
for (i in 1:NROW(confusion_tables_chiapet_merged)){
  confusion_tables_chiapet_merged[i]=list(caret::confusionMatrix(factor(common_chiapet_manorm_merged[[names(confusion_tables_chiapet_merged)[i]]]$common_padj, levels=c(TRUE, FALSE)), 
                       factor(common_chiapet_manorm_merged[[names(confusion_tables_chiapet_merged)[i]]]$predicted_rev, levels=c(TRUE, FALSE)),
                       dnn=c("padj>0.001", "SPP_detected")))
}


# pos_false_tables_hichip_merged$GM38_GM39_mean_gr10$GM38T_GM38F[pos_false_tables_hichip_merged$GM38_GM39_mean_gr10$GM38T_GM38F$end-pos_false_tables_hichip$GM38_GM39_mean_gr10$GM38T_GM38F$start<=5000,]

# 4 cases
confusion_data_chiapet_merged=list(
  GM19238_GM19239=list(
    # Oba pokazują tak (TP)
    TP=pos_false_tables_chiapet_merged$GM38_GM39$GM38T_GM39T[pos_false_tables_chiapet_merged$GM38_GM39$GM38T_GM39T$common_padj==TRUE,],
    # Spp pokazuje tak, a padj pokazuje róźną siłę (FP)                                 
    FP=pos_false_tables_chiapet_merged$GM38_GM39$GM38T_GM39T[pos_false_tables_chiapet_merged$GM38_GM39$GM38T_GM39T$common_padj==FALSE,],
    # SPP pokazuje nie, a padj pokazuje, że siła jest taka sama (FN)
    FN=pos_false_tables_chiapet_merged$GM38_GM39$false_peaks[pos_false_tables_chiapet_merged$GM38_GM39$false_peaks$common_padj==TRUE,],
    # SPP pokazuje nie i padj się zgadza  (TN)
    TN=pos_false_tables_chiapet_merged$GM38_GM39$false_peaks[pos_false_tables_chiapet_merged$GM38_GM39$false_peaks$common_padj==FALSE,]),
  GM19238_GM19240=list(
    TP=pos_false_tables_chiapet_merged$GM38_GM40$GM38T_GM40T[pos_false_tables_chiapet_merged$GM38_GM40$GM38T_GM40T$common_padj==TRUE,],
    # Spp pokazuje tak, a padj pokazuje róźną siłę (FP)
    FP=pos_false_tables_chiapet_merged$GM38_GM40$GM38T_GM40T[pos_false_tables_chiapet_merged$GM38_GM40$GM38T_GM40T$common_padj==FALSE,],
    # SPP pokazuje nie, a padj pokazuje, że siła jest taka sama (FN)
    FN=pos_false_tables_chiapet_merged$GM38_GM40$false_peaks[pos_false_tables_chiapet_merged$GM38_GM40$false_peaks$common_padj==TRUE,],
    # SPP pokazuje nie i padj się zgadza  (TN)
    TN=pos_false_tables_chiapet_merged$GM38_GM40$false_peaks[pos_false_tables_chiapet_merged$GM38_GM40$false_peaks$common_padj==FALSE,]),
  GM19239_GM19240=list(
    TP=pos_false_tables_chiapet_merged$GM39_GM40$GM39T_GM40T[pos_false_tables_chiapet_merged$GM39_GM40$GM39T_GM40T$common_padj==TRUE,],
    # Spp pokazuje tak, a padj pokazuje róźną siłę (FP)
    FP=pos_false_tables_chiapet_merged$GM39_GM40$GM39T_GM40T[pos_false_tables_chiapet_merged$GM39_GM40$GM39T_GM40T$common_padj==FALSE,],
    # SPP pokazuje nie, a padj pokazuje, że siła jest taka sama (FN)
    FN=pos_false_tables_chiapet_merged$GM39_GM40$false_peaks[pos_false_tables_chiapet_merged$GM39_GM40$false_peaks$common_padj==TRUE,],
    # SPP pokazuje nie i padj się zgadza  (TN)
    TN=pos_false_tables_chiapet_merged$GM39_GM40$false_peaks[pos_false_tables_chiapet_merged$GM39_GM40$false_peaks$common_padj==FALSE,]))
