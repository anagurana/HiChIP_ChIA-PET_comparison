...
# [ChIA-PET merged] Creating additional columns with occupancy based on overlaps
```{r}

trios=c("GM19238", "GM19239", "GM19240")

# # Create overlapping GRanges with bins (all of the peaks are overlapped)
# 
# overlaps_chiapet_with_bins.gr=GRangesList(GRanges(),GRanges(),GRanges())
# names(overlaps_chiapet_with_bins.gr)=trios
# for (cell_line in trios){
#   overlaps_chiapet_with_bins.gr[[cell_line]]=subsetByOverlaps(chiapet_bins,peaks_with_motif_ChIAPET[[cell_line]])
# }
# 
# # Convert those GRanges to data frames
# 
# overlaps_chiapet_with_bins.df=list()
# for (cell_line in trios){
#   overlaps_chiapet_with_bins.df[[cell_line]]=data.frame(overlaps_chiapet_with_bins.gr[[cell_line]])
#   
#   
# }

# Calculate occupancy by overlapping with selected peaks bins
...
png(paste0('plots/',experiment,'_MAnorm_merRep_trios_diff_test_pval_',p_value,'_overlaps_trial.png'), res = 300, width = 2200, height = 2200)
par(mar=c(2.3,2.3,2.3,2.3))
layout(matrix(c(1,2,3,4,1,5,3,4,1,6,3,4),ncol=3),heights=c(1,3,1,3,1,3))
...
peaks_excl_3_cell_line=list()
names(peaks_excl_3_cell_line)=create_short_name_for_col(trios)

# TODO create list with filtered peaks in order to create confusion matrices

for (pair in names(peaks_excl_3_cell_line)){
  peaks_excl_3_cell_line[pair]=
}
peaks_excl_3_cell_line=list(
...
cos=anti_join(common_chiapet_manorm$GM38_GM39, common_chiapet_manorm$GM38_GM39[common_chiapet_manorm$GM38_GM39$GM19238.occupancy==0 & common_chiapet_manorm$GM38_GM39$GM19239.occupancy==0,])
cos=anti_join(cos, cos[cos$chrom=="chrX" | cos$chrom=="chrY",])

conf_38_39=caret::confusionMatrix(factor(cos$common_pval, levels=c(TRUE, FALSE)), 
                       factor(cos$predicted_rev, levels=c(TRUE, FALSE)),
                       dnn=c("pval>0.03", "SPP_detected"))
...
data.frame(chiapet_bins)$start==overlaps_chiapet_with_bins.df$GM19238$start & data.frame(chiapet_bins)$end==overlaps_chiapet_with_bins.df$GM19238$end
...
pos_false_table=pos_false_tables_chiapet
...
### Duplicate the peaks
# 9677 peaks are constant and were supposed to be unique for PAIRS of cell lines
id_constant_chiapet_by_manorm_peaks_pairs=unique(unlist(chiapet_fn_ids[1:3]))
# 10799 peaks are constant and were supposed to be unique for cell lines
id_constant_chiapet_by_manorm_peaks_single_lines=unique(unlist(chiapet_fn_ids[4:6]))

constant_chiapet_by_manorm_peaks=norm_chiapet[c(id_constant_chiapet_by_manorm_peaks_pairs, 
...
# 9297 of 20476 are overlaping with ChIP-Seq
subsetByOverlaps(constant_chiapet_by_manorm_peaks.gr, true_common_peaks_ChIPSeq$constant_peaks)
# 3364 of 20476 are actually overlaping with SPP detected peaks, but for some reason had false occupancy
subsetByOverlaps(constant_chiapet_by_manorm_peaks.gr, true_common_peaks_ChIAPET$constant_peaks)
# That's why we want to reduce these peaks. In total we have 40174 peaks, after reduction 36810
...
# Let's do the overlap of all true peaks with ChIP-Seq constant peaks
# 22291 from 36810 were overlapped (before only 15600, so +6691 new peaks), ChIP-Seq have 37332 peaks. 
